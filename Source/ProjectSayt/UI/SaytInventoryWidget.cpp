...
void USaytInventoryWidget::SetInventoryComponent(USaytInventoryComponent* InInventoryComponent)
{
...
void USaytInventoryWidget::BuildGrid()
{
	...
	for (int32 Y = 0; Y < Height; ++Y)
	{
		...
		{
			USaytInventorySlotWidget* NewSlot = CreateWidget<USaytInventorySlotWidget>(this, SlotWidgetClass);
			if (!NewSlot)
			{
				continue;
			}
...
void USaytInventoryWidget::RefreshItems()
{
	...
	for (const FSaytInventoryEntry& Entry : InventoryComponent->GetEntries())
	{
		...
		USaytInventoryItemWidget* NewItemWidget = CreateWidget<USaytInventoryItemWidget>(this, ItemWidgetClass);
		if (!NewItemWidget)
		{
			continue;
		}
...
void USaytInventoryWidget::HandleItemDropped(USaytItemInstance* Instance, FIntPoint NewTopLeft, FIntPoint OriginalTopLeft)
{
