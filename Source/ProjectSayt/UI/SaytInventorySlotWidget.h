...
#include "SaytInventorySlotWidget.generated.h"
...
UENUM(BlueprintType)
enum class ESaytPlacementHighlight : uint8
{
	None,    // 평상시 — 하이라이트 없음
	Valid,   // 배치 가능 (녹색)
	Invalid  // 배치 불가 (빨강)
 ...
 */
UCLASS()
class PROJECTSAYT_API USaytInventorySlotWidget : public UCommonUserWidget
{
	GENERATED_BODY()
	
public:
	// 드래그 중 이 칸의 하이라이트 상태를 외부(그리드 컨테이너)에서 지정.
	void SetHighlight(ESaytPlacementHighlight NewState) const;
	...
	void SetOwningInventoryWidget(USaytInventoryWidget* InOwner) { OwningInventoryWidget = InOwner; }
...
private:
	...
	UPROPERTY()
	TObjectPtr<USaytInventoryWidget> OwningInventoryWidget;
};
