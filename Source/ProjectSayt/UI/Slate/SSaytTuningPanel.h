...
DECLARE_DELEGATE_OneParam(FOnSaytDragValueChanged, float)

class SSaytDragTrack : public SCompoundWidget
{
	public:
	SLATE_BEGIN_ARGS(SSaytDragTrack)
		: _Value(0.f)
		{}
		SLATE_ATTRIBUTE(float, Value)
		SLATE_EVENT(FOnSaytDragValueChanged, OnValueChanged)
	SLATE_END_ARGS()
...
private:
	...
	FOnSaytDragValueChanged OnValueChanged;
	bool bIsDragging;
};

// 실제로 화면에 띄울 패널입니다. Model을 직접 들고 있고,
// 자식 위젯(ProgressBar/TextBlock)은 전부 그 Model을 TAttribute로 Pull합니다.
class PROJECTSAYT_API SSaytTuningPanel : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SSaytTuningPanel) {}
	SLATE_END_ARGS()
