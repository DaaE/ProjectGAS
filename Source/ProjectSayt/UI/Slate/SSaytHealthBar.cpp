...
static TSharedPtr<SWidget> GSaytHealthBarRoot;
static TWeakPtr<SSaytHealthBar> GSaytHealthBarWidget;

static FAutoConsoleCommandWithWorld GSaytHealthBarShowCmd(
	TEXT("Sayt.HealthBar.Show"),
	TEXT("[임시] 플레이어 ASC에 바인딩된 SSaytHealthBar를 좌하단에 토글"),
	FConsoleCommandWithWorldDelegate::CreateLambda([](UWorld* World)
	{
		UGameViewportClient* Viewport = World ? World->GetGameViewport() : nullptr;
		if (!Viewport) { return; }

		if (GSaytHealthBarRoot.IsValid())
		{
			Viewport->RemoveViewportWidgetContent(GSaytHealthBarRoot.ToSharedRef());
			GSaytHealthBarRoot.Reset();
			return;
		}
		...
		Viewport->AddViewportWidgetContent(GSaytHealthBarRoot.ToSharedRef());
		Bar->BindToASC(ASC);
		GSaytHealthBarWidget = Bar;
	})
);

// 신규 — [임시] 줄 색 스왑 시뮬레이션: Sayt.HealthBar.Tint <R> <G> <B> (0~1)
static FAutoConsoleCommandWithWorldAndArgs GSaytHealthBarTintCmd(
	...
	{
		TSharedPtr<SSaytHealthBar> Bar = GSaytHealthBarWidget.Pin();
...
static FDelegateHandle GSaytHealthBarCleanup = FWorldDelegates::OnWorldCleanup.AddLambda(
	[](UWorld*, bool, bool) { GSaytHealthBarRoot.Reset(); });
#endif
