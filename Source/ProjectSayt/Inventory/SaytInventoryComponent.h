...
#include "SaytInventoryComponent.generated.h"

class USaytItemInstance;
class USaytItemDefinition;

// 그리드에 배치된 아이템 한 개를 나타냄.
USTRUCT(BlueprintType)
struct FSaytInventoryEntry
{
	GENERATED_BODY()
	
	UPROPERTY()
	TObjectPtr<USaytItemInstance> Instance = nullptr;
 ...
 */
UCLASS()
class PROJECTSAYT_API USaytInventoryComponent : public UActorComponent
{
	GENERATED_BODY()

public:
	// Sets default values for this component's properties
	USaytInventoryComponent();

	// 지정 위치에 놓을 수 있는지 판정만 함 (실제로 놓지 않음).
	// 풋프린트 전체가 비어있거나, 동일한 스택 가능 아이템과 정확히 겹치면 true.
	bool CanPlaceItemAt(USaytItemDefinition* ItemDef, FIntPoint TopLeft, USaytItemInstance* IgnoreInstance = nullptr) const;

	// 지정 위치에 Instance 배치 시도 (드래그&드롭처럼 좌표가 확정된 경우용).
	// 호환되는 기존 스택과 정확히 겹치면 병합, 비어있으면 새로 배치.
	bool TryAddInstanceAt(USaytItemInstance* Instance, FIntPoint TopLeft);
	
	// 빈 자리를 좌상단부터 자동으로 찾아서 추가 시도.
	// 이미 존재하는 Instance를 추가 시도. 같은 종류의 스택 가능한 슬롯이 있으면
	// 먼저 그쪽에 (부분적으로라도) 합치고, 남는 수량은 빈 자리에 새로 배치.
	bool TryAddInstance(USaytItemInstance* Instance);

	void RemoveItem(USaytItemInstance* Instance);
	...
	const TArray<FSaytInventoryEntry>& GetEntries() const { return Entries; }
...
protected:
	...
	UPROPERTY(EditDefaultsOnly, Category = "Sayt|Inventory")
	int32 GridWidth = 10;
...
private:
	...
	bool GetUniformFootprintOccupant(FIntPoint TopLeft, FIntPoint Size, USaytItemInstance*& OutOccupant, USaytItemInstance* IgnoreInstance = nullptr) const;
	
	// 특정 칸의 "사실상" 점유자. IgnoreInstance와 같으면 빈 칸(nullptr)인 것처럼 취급 —
	// 드래그 호버 판정에서 "자기 자신이 차지한 칸"을 무시하기 위한 용도.
	USaytItemInstance* GetEffectiveOccupant(int32 X, int32 Y, USaytItemInstance* IgnoreInstance) const;
	...
	TArray<TObjectPtr<USaytItemInstance>> CellOccupancy;

	UPROPERTY()
	TArray<FSaytInventoryEntry> Entries;
};
