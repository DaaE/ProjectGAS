...
#include "SaytInventoryComponent.h"

#include "SaytLogChannels.h"
#include "Items/SaytItemDefinition.h"
#include "Items/SaytItemIdSubsystem.h"
#include "Items/SaytItemInstance.h"


// Sets default values for this component's properties
USaytInventoryComponent::USaytInventoryComponent()
{
...
void USaytInventoryComponent::BeginPlay()
{
	Super::BeginPlay();
...
bool USaytInventoryComponent::GetUniformFootprintOccupant(FIntPoint TopLeft, FIntPoint Size, USaytItemInstance*& OutOccupant, USaytItemInstance* IgnoreInstance) const
{
...
USaytItemInstance* USaytInventoryComponent::GetEffectiveOccupant(int32 X, int32 Y, USaytItemInstance* IgnoreInstance) const
{
	USaytItemInstance* Cell = CellOccupancy[Y * GridWidth + X];
	return (Cell == IgnoreInstance) ? nullptr : Cell;
}

bool USaytInventoryComponent::CanPlaceItemAt(USaytItemDefinition* ItemDef, FIntPoint TopLeft, USaytItemInstance* IgnoreInstance) const
{
	if (!ItemDef)
	{
		return false;
	...
	USaytItemInstance* Occupant = nullptr;
...
bool USaytInventoryComponent::TryAddInstanceAt(USaytItemInstance* Instance, FIntPoint TopLeft)
{
	...
	USaytItemDefinition* ItemDef = Instance->ItemDefinition;
	const FIntPoint Size = ItemDef->GridSize;
	...
	if (Occupant != nullptr)
	{
		...
		const int32 EntryIndex = Entries.IndexOfByPredicate(
			[Occupant](const FSaytInventoryEntry& Entry) { return Entry.Instance == Occupant; });
...
bool USaytInventoryComponent::TryAddInstance(USaytItemInstance* Instance)
{
	if (!Instance || !Instance->ItemDefinition)
	{
		return false;
	...
	{
		for (FSaytInventoryEntry& Entry : Entries)
		{
...
void USaytInventoryComponent::RemoveItem(USaytItemInstance* Instance)
{
	const int32 EntryIndex = Entries.IndexOfByPredicate(
		[Instance](const FSaytInventoryEntry& Entry) { return Entry.Instance == Instance; });
...
bool USaytInventoryComponent::ExpandGrid(int32 NewWidth, int32 NewHeight)
{
	...
	for (const FSaytInventoryEntry& Entry : Entries)
	{
...
void USaytInventoryComponent::DebugPrintGrid() const
{
	...
	UE_LOG(LogSaytData, Verbose, TEXT("%s"), *GridString);
}
