...
struct FSaytDamageStatics
{
	...
	FSaytDamageStatics()
	{
		...
		DEFINE_ATTRIBUTE_CAPTUREDEF(USaytAttributeSet_Combat, AttackPower, Source, false);
		// 피격자(Target)의 Defense - MMC로는 못 했던 부분
		DEFINE_ATTRIBUTE_CAPTUREDEF(USaytAttributeSet_Combat, Defense, Target, false);
	}
};

static const FSaytDamageStatics& DamageStatics()
{
	static FSaytDamageStatics Statics;
	return Statics;
}

USaytExecCalc_Damage::USaytExecCalc_Damage()
{
	RelevantAttributesToCapture.Add(DamageStatics().AttackPowerDef);
	RelevantAttributesToCapture.Add(DamageStatics().DefenseDef);
}

void USaytExecCalc_Damage::Execute_Implementation(
