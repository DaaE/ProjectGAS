...
#include "SaytAbility_BasicAttack.h"
...
USaytAbility_BasicAttack::USaytAbility_BasicAttack()
{
...
void USaytAbility_BasicAttack::ActivateAbility(
	...
	const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
	...
	WaitWindowOpenTask->EventReceived.AddDynamic(this, &USaytAbility_BasicAttack::OnComboWindowOpenEvent);
	...
	WaitWindowCloseTask->EventReceived.AddDynamic(this, &USaytAbility_BasicAttack::OnComboWindowCloseEvent);
	...
	WaitComboInputTask->EventReceived.AddDynamic(this, &USaytAbility_BasicAttack::OnComboInputEvent);
	...
	WaitTraceStartTask->EventReceived.AddDynamic(
		this, &USaytAbility_BasicAttack::OnTraceStartEvent);
	...
	WaitTraceEndTask->EventReceived.AddDynamic(
		this, &USaytAbility_BasicAttack::OnTraceEndEvent);
	...
	WaitAttackVoiceTask->EventReceived.AddDynamic(this, &USaytAbility_BasicAttack::OnAttackVoiceEvent);
...
void USaytAbility_BasicAttack::EndAbility(const FGameplayAbilitySpecHandle Handle,
...
UGameplayEffect* USaytAbility_BasicAttack::GetCooldownGameplayEffect() const
{
...
void USaytAbility_BasicAttack::OnMontageCompleted()
{
	UE_LOG(LogSaytCombat, Verbose, TEXT("Montage Completed - Normal End"));
	// "정상 종료" - 여기서 쿨타임을 정식으로 시작해도 됨
	EndAbility(GetCurrentAbilitySpecHandle(), CurrentActorInfo, 
		GetCurrentActivationInfo(), true, false);
}

void USaytAbility_BasicAttack::OnMontageInterrupted()
{
	UE_LOG(LogSaytCombat, Verbose, TEXT("Montage Interrupted - Cancelled"));
	// "비정상 캔슬" - 다른 어빌리티가 끼어들어 중단된 경우
	EndAbility(GetCurrentAbilitySpecHandle(), CurrentActorInfo,
		GetCurrentActivationInfo(), true, true);
}

void USaytAbility_BasicAttack::OnMontageCancelled()
{
	UE_LOG(LogSaytCombat, Verbose, TEXT("Montage Cancelled"));
	EndAbility(GetCurrentAbilitySpecHandle(), CurrentActorInfo,
		GetCurrentActivationInfo(), true, true);
}

void USaytAbility_BasicAttack::OnComboWindowOpenEvent(FGameplayEventData Payload)
{
	UE_LOG(LogSaytCombat, Verbose, TEXT("Combo Window OPEN event received"));
...
void USaytAbility_BasicAttack::OnComboWindowCloseEvent(FGameplayEventData Payload)
{
	UE_LOG(LogSaytCombat, Verbose, TEXT("Combo Window CLOSE event received"));
...
void USaytAbility_BasicAttack::OnComboInputEvent(FGameplayEventData Payload)
{
...
void USaytAbility_BasicAttack::PlayComboStep(int32 Index)
{
	CurrentComboIndex = Index;
	UE_LOG(LogSaytCombat, Verbose, TEXT("Combo Hit: %d"), Index + 1);
	...
	MontageTask->OnCompleted.AddDynamic(this, &USaytAbility_BasicAttack::OnMontageCompleted);
	MontageTask->OnInterrupted.AddDynamic(this, &USaytAbility_BasicAttack::OnMontageInterrupted);
	MontageTask->OnCancelled.AddDynamic(this, &USaytAbility_BasicAttack::OnMontageCancelled);
	MontageTask->ReadyForActivation();
}

void USaytAbility_BasicAttack::OnTraceStartEvent(FGameplayEventData Payload)
{
	UE_LOG(LogSaytCombat, Verbose, TEXT("[Combo %d] TraceStart - Task 생성"), CurrentComboIndex);
	
	// 혹시 직전 Task가 안 끝나고 남아있으면 정리 (안전장치)
	if (WeaponTraceTask)
	{
		UE_LOG(LogSaytCombat, Verbose, TEXT("[Combo %d] 이전 Task 정리"), CurrentComboIndex);
		WeaponTraceTask->EndTask();
	}
	...
	WeaponTraceTask->OnHit.AddDynamic(
		this, &USaytAbility_BasicAttack::OnWeaponHitActor);
	WeaponTraceTask->ReadyForActivation();
}

void USaytAbility_BasicAttack::OnTraceEndEvent(FGameplayEventData Payload)
{
	UE_LOG(LogSaytCombat, Verbose, TEXT("[Combo %d] TraceEnd - Task 종료"), CurrentComboIndex);
...
void USaytAbility_BasicAttack::OnWeaponHitActor(AActor* HitActor, FHitResult Hit)
{
	UE_LOG(LogSaytCombat, Verbose, TEXT("[Combo %d] OnWeaponHitActor 호출: %s"),
	...
	if (EffectSpec.IsValid())
	{
		...
		UE_LOG(LogSaytCombat, Verbose, TEXT("[Combo %d] Multiplier: %.2f, ComboDamageMultipliers.Num(): %d"),
...
void USaytAbility_BasicAttack::OnAttackVoiceEvent(FGameplayEventData Payload)
{
	...
	if (ASaytEffectPoolManager* PoolManager = Cast<ASaytEffectPoolManager>(
		UGameplayStatics::GetActorOfClass(
			GetWorld(), ASaytEffectPoolManager::StaticClass())))
	{
