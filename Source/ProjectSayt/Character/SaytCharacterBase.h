...
#include "SaytCharacterBase.generated.h"
...
class USaytGameplayAbility;
class USaytAttributeSet_Combat;

// 플레이어/적 등 GAS를 쓰는 모든 Character가 공유하는 베이스.
// ASC 생성, 어빌리티 부여, 초기화 타이밍까지 여기서 전담한다.
UCLASS()
class PROJECTSAYT_API ASaytCharacterBase : public ACharacter, public IAbilitySystemInterface
// PROJECTSAYT_API : DLL export 매크로, 다른 모듈에서 이 클래스를 쓸 수 있게 함
...
protected:
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Abilities")
	TObjectPtr<USaytAttributeSet_Combat> CombatAttributeSet;

	// 이 캐릭터가 SayuGameDataSubsystem의 DataTable에서 어떤 행을 쓸지 식별.
	// 자식 클래스(ASaytCharacter/ASaytNPCCharacter) 생성자에서 각자 다르게 설정.
	...
	TArray<TSubclassOf<USaytGameplayAbility>> DefaultAbilities;
