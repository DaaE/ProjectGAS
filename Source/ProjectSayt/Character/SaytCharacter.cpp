...
#include "UI/SaytInventoryWidget.h"
#include "UI/SaytUIInputModeSubsystem.h"


// Sets default values
ASaytCharacter::ASaytCharacter()
...
// 컴포넌트 생성과 기본값 설정은 여기서
{
	...
	InventoryComponent = CreateDefaultSubobject<USaytInventoryComponent>(TEXT("InventoryComponent"));
}

void ASaytCharacter::OnBasicAttackInput(const struct FInputActionValue& Value)
{
...
void ASaytCharacter::OnBuffInput(const struct FInputActionValue& Value)
{
	if (AbilitySystemComponent)
	{
		FGameplayTagContainer TagContainer;
...
void ASaytCharacter::OnQuickSaveInput(const struct FInputActionValue& Value)
{
	if (USaytGameDataSubsystem* GameData = GetGameInstance()->GetSubsystem<USaytGameDataSubsystem>())
	{
		// CombatAttributeSet은 ASaytCharacterBase의 protected 멤버라 자식 클래스에서 바로 접근 가능
		GameData->SaveCombatState(CombatAttributeSet, TEXT("QuickSave"));
	}
}

void ASaytCharacter::OnQuickLoadInput(const FInputActionValue& Value)
{
	if (USaytGameDataSubsystem* GameData = GetGameInstance()->GetSubsystem<USaytGameDataSubsystem>())
	{
...
void ASaytCharacter::OnDebugDamageInput(const struct FInputActionValue& Value)
{
	...
	if (AbilitySystemComponent && DebugSelfDamageEffectClass)
	{
		...
		if (Spec.IsValid())
		{
			...
			UE_LOG(LogSaytCombat, Log, TEXT("[Debug] Self-damage GE applied"));
		}
	}
}

void ASaytCharacter::DebugEquipTestItem()
{
	if (!DebugTestItem)
	{
		return;
	...
	for (const TObjectPtr<USaytItemFragment>& Fragment : DebugTestItem->Fragments)
	{
		if (Fragment)
		{
	...
	{
		if (USaytItemInstance* NewInstance = USaytItemInstance::CreateInstance(this, DebugTestItem))
		{
...
void ASaytCharacter::DebugToggleInventory()
{
	...
	if (ActiveInventoryWidget)
	{
		...
		{
			GI->GetSubsystem<USaytUIInputModeSubsystem>()->PopUIRequest(TEXT("Inventory"));
		}
	...
	{
		ActiveInventoryWidget = CreateWidget<USaytInventoryWidget>(GetWorld(), InventoryWidgetClass);
		...
		if (UGameInstance* GI = GetGameInstance())
		{
			GI->GetSubsystem<USaytUIInputModeSubsystem>()->PushUIRequest(TEXT("Inventory"));
		}
	}
}

void ASaytCharacter::OnUIMousePressed()
{
...
void ASaytCharacter::OnUIMouseReleased()
{
	if (APlayerController* PC = Cast<APlayerController>(GetController()))
	{
...
void ASaytCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
...
void ASaytCharacter::Look(const FInputActionValue& Value)
{
