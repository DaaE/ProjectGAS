...
#include "SaytEffectPoolManager.generated.h"

class ASaytPooledEffect;

// 타격 이펙트 액터를 미리 여러 개 만들어두고 빌려주는 풀 매니저.
// Phase 4에서 Subsystem으로 전환할 걸 염두에 두고, 풀링 로직을
// 이 클래스 안에 최대한 캡슐화해서 나중에 그대로 옮기기 쉽게 함.
UCLASS()
class PROJECTSAYT_API ASaytEffectPoolManager : public AActor
{
	GENERATED_BODY()
	
public:	
	// Sets default values for this actor's properties
	ASaytEffectPoolManager();
...
protected:
	...
	TSubclassOf<ASaytPooledEffect> EffectClass;
...
private:
	UPROPERTY()
	TArray<TObjectPtr<ASaytPooledEffect>> Pool;

	ASaytPooledEffect* SpawnPooledEffect();
};
